                ...
                match event {
                    ...
                    Event::Circle(circle) => unsafe {
                        ...
                        if let Some(center) = geometry::circumcenter(pf1, pf2, pf3) {
                            ...
                            debug_assert_eq!(self.graph.topo.verts.len(), vert_idx);
                            ...
                            self.graph.max_rsq = self.graph.max_rsq.max((center - pf1).norm_sq());
                        } else {
                            debug_assert_eq!(self.graph.topo.half_edges[i_he1].vert, graph::INF);
                            debug_assert_eq!(self.graph.topo.half_edges[i_he2].vert, graph::INF);
                        }
                    },
                }
                Node::validate_parents(self.beachline);
