...
fn cycle_decomp(mut perm: Vec<u8>) -> Vec<u8> {
    let mut cs = vec![];
    for mut u in 0..perm.len() as u8 {
        if perm[u as usize] == UNSET {
            continue;
        }

        let mut size = 0;
        loop {
            u = std::mem::replace(&mut perm[u as usize], UNSET);
            if u == UNSET {
                break;
            }
            size += 1;
...
fn main() {
    ...
    let (fc, ifc) = gen_factorials::<M>(n as u32);

    let partitions = {
        let mut bounded_partitions = vec![vec![vec![]; n + 1]; n + 1];
        bounded_partitions[0][0].push(vec![]);
        for s in 1..=n {
            for b in 0..=s {
                for k in (1..=b).rev() {
                    let base =
                        bounded_partitions[(s - k) as usize][(s - k).min(k) as usize].clone();
                    bounded_partitions[s][b].extend(base.into_iter().map(|mut t| {
                        t.push(k as u8);
                        t
                    }));
                }
        ...
        std::mem::take(&mut bounded_partitions[n][n])
    };
    let ord = partitions
        .iter()
        .enumerate()
        .map(|(u, ps)| (ps.iter().copied().collect::<Box<[u8]>>(), u as u16))
        .collect::<HashMap<_, _>>();

    let r = partitions.len();
    let mut transitions = vec![vec![]; r];
    for (u, ps) in partitions.iter().enumerate() {
        let m = ps.len();
        ...
        for i in 0..m {
            ...
            for y in 1..=x / 2 {
                let z = x - y;
                let w = M::from(if y == z { y } else { x });
                let mut qs: Vec<_> = ps[..i]
                    .iter()
                    .chain(&ps[i + 1..])
                    .copied()
                    .chain([y, z])
                    .collect();
                qs.sort();
                transitions[u].push((ord[qs.as_slice()], w));
            }
        }
        for i in 0..m {
            for j in i + 1..m {
                let x = ps[i] + ps[j];
                let w = M::from(ps[i] as u32 * ps[j] as u32);
    ...
    let mut dp = vec![M::zero(); r];
    dp[ord[vec![1; n].as_slice()] as usize] = ifc[n];
    for _ in 0..k {
        let prev = dp;
        dp = vec![M::zero(); r];
        for u in 0..r {
            for &(v, w) in &transitions[u] {
                dp[v as usize] += prev[u as usize] * w;
            }
        }
    }
    for (u, ps) in partitions.iter().enumerate() {
        ...
        for s in 1..=n {
            if freq[s] >= 1 {
                dp[u] *= fc[freq[s] as usize];
            }
        }
    }

    for _ in 0..t {
        let perm = (0..n).map(|_| input.value::<u8>() - 1).collect();
        let ps = cycle_decomp(perm);
        writeln!(output, "{}", dp[ord[ps.as_slice()] as usize]).unwrap();
    }
}
