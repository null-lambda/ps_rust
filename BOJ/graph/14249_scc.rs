...
fn main() {
    ...
    for i in 0..n {
        let step: isize = input.value();
        for v in [i as isize - step, i as isize + step] {
            if !(0..n as isize).contains(&v) {
                continue;
            }
            children[i].push(v as u32);
        }
    }

    let start = input.value::<usize>() - 1;

    let children = Jagged::from_iter(children);
    let (scc_count, scc_index) = gen_scc(&children);
    let mut parent_comps = vec![vec![]; scc_count];
    let mut size = vec![0; scc_count];
    for u in 0..n {
        size[scc_index[u] as usize] += 1;
        for &v in &children[u] {
            parent_comps[scc_index[v as usize] as usize].push(scc_index[u as usize]);
        }
    }

    let mut dp = vec![0u32; scc_count];
    for u in 0..scc_count {
        dp[u] += size[u] as u32;
        for &p in &parent_comps[u] {
            dp[p as usize] = dp[p as usize].max(dp[u]);
        }
    }
    let ans = dp[scc_index[start] as usize];
