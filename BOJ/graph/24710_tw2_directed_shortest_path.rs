...
fn floyd_warshall(m: usize, mat: &mut Mat) {
    for k in 0..m {
        for i in 0..m {
            for j in 0..m {
...
impl reroot::DpSpec for InnerDist {
    ...
    fn finalize(&self, (bag, mat): &mut Self::V) {
        floyd_warshall(bag_len(&bag), mat);
    }
}
...
impl BagDist {
    fn rev_edges(&self, path: &<Self as Monoid>::X) -> <Self as Monoid>::X {
        path.map(|(ends, mat)| {
    ...
    fn rev(&self, path: &<Self as Monoid>::X) -> <Self as Monoid>::X {
        self.rev_edges(path).map(|(mut ends, mat)| {
            ends.swap(0, 1);
            (ends, mat)
        })
    }
...
fn main() {
    ...
    for _ in 0..input.value() {
        ...
        let hs: Vec<_> = (0..n).map(|_| input.value::<u32>()).collect();
        let hs_max = SegTree::from_iter(hs.iter().copied(), MaxOp);

        let (parent, children) = max_cartesian_tree(n, hs.iter().copied().enumerate());
        let mut alt_parent = vec![UNSET; n];

        let ws: Vec<[u32; 2]> = (0..n)
        ...
        let mut timer = 0;
        let mut add_edge = |mut u: u32, mut v: u32| {
            if u > v {
                std::mem::swap(&mut u, &mut v)
            };

            if hs_max.sum_range(u as usize + 1..v as usize) >= hs[u as usize].min(hs[v as usize]) {
                return;
            }
            edges.insert([u, v], ws[u as usize][1]);
            edges.insert([v, u], ws[v as usize][0]);
        };

        while let Some(&u) = bfs.get(timer) {
            let a = alt_parent[u as usize];
            ...
            if p != UNSET {
                add_edge(u, p);
                let branch = (children[p as usize][1] == u) as usize;
                if cs[branch] != UNSET {
                    alt_parent[cs[branch] as usize] = a;
                }
                if cs[branch ^ 1] != UNSET {
                    alt_parent[cs[branch ^ 1] as usize] = p;
                }
            }
        ...
        for u in 0..n {
            ...
            for i in 0..m {
                for j in 0..m {
                    mat[i][j] = if i == j {
                        0
                    } else if let Some(&w) = edges.get(&[bag[i], bag[j]]) {
                        w as u64
                    } else {
                        INF
                    };
        ...
        let weights_rev = weights.iter().map(|w| BagDist.rev(w));
        let st_rev = segtree::SegTree::from_iter(weights_rev, BagDist);
        ...
        for _ in 0..q {
            ...
            hld.for_each_path_splitted(u, v, |u, v, on_left, _| {
                if on_left {
                    lhs = BagDist.op(&st_rev.sum_range(sid(u)..sid(v) + 1), &lhs);
                } else {
                    rhs = BagDist.op(&st.sum_range(sid(u)..sid(v) + 1), &rhs);
                }
            });
