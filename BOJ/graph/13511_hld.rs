...
fn main() {
    ...
    let hld = hld::HLD::from_edges(n, edges.iter().map(|&(u, v, _)| (u, v)), 0, false);
    ...
    depth[0] = 0;
    for &(u, v, w) in &edges {
        let bot = if sid(u as usize) > sid(v as usize) {
            u
        } else {
            v
        };
        dist[bot as usize] = w as u64;
    }

    for &u in hld.topological_order.iter().rev().skip(1) {
        let p = hld.parent[u as usize] as usize;
        dist[u as usize] += dist[p];
        depth[u as usize] += depth[p];
    }
    ...
    for _ in 0..input.value() {
        ...
        let u = input.value::<u32>() - 1;
        let v = input.value::<u32>() - 1;
        let j = hld.lca(u as usize, v as usize);
        match cmd {
            "1" => {
                let ans = dist[u as usize] + dist[v as usize] - 2 * dist[j];
                writeln!(output, "{}", ans).unwrap();
            }
            "2" => {
                let mut k = input.value::<u32>() - 1;
                let du = depth[u as usize] - depth[j];
                let dv = depth[v as usize] - depth[j];
                assert!(k <= du + dv);
                ...
                loop {
                    let top = hld.chain_top[c as usize] as usize;
                    let d = depth[c as usize] - depth[top];
                    if k <= d {
                        c = sid_inv[sid(c as usize) - k as usize];
                        break;
                    }
                    c = hld.parent[top];
                    k -= d + 1;
