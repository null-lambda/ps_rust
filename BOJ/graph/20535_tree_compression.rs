...
impl NodeData {
    ...
    fn pull_from(&mut self, child: &NodeData, inv: bool) {
        if !inv {
            self.count_pair += self.count * child.count;
    ...
    fn finalize(&self, d: i64, ans: &mut i64) {
        *ans += self.count_pair * d;
    }
}
...
fn main() {
    ...
    let edges = (1..n as u32).map(|u| (u, input.value::<u32>() - 1));
    ...
    for _ in 0..q {
        ...
        for i in (1..relevant.len()).rev() {
            ...
            let dp_u = dp[u].clone();
            dp_u.finalize(depth[u] as i64, &mut ans);
            dp[p].pull_from(&dp_u, false);
        }
        dp[r].finalize(depth[r] as i64, &mut ans);
