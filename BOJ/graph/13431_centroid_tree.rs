...
fn build_depth<'a>(
    neighbors: &'a impl Jagged<'a, (u32, u64)>,
    dist: &mut [u64],
    u: usize,
    p: usize,
) {
    for &(v, w) in neighbors.get(u) {
        ...
        build_depth(neighbors, dist, v as usize, u);
    }
}
...
fn main() {
    ...
    let mut sum = vec![0; n]; // sum dist to u - sum dist to parent if exists
    let mut subtree_count = vec![0; n];
        ...
        match input.token() {
            ...
            "2" => {
                ...
                loop {
                    ...
                    let d_u0p = dist(u0, p);
                    let c_p = subtree_count[p] - subtree_count[u];
                    ans += c_p * d_u0p;
