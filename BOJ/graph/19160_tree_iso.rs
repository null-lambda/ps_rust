...
fn main() {
    ...
    let edges = (1..n as u32).map(|_| (input.value::<u32>() - 1, input.value::<u32>() - 1, ()));
    let comp = tree_iso::AHULabelCompression::from_edges(n, edges, None);

    let ans = || {
        let &[centroid] = comp.forest_roots.0.as_slice() else {
            return None;
        };

        let mut subtrees = vec![];
        for &(c, ()) in &comp.ordered_children[centroid] {
            let mut edges = vec![];
            let mut stack = vec![c as u32];
            while let Some(u) = stack.pop() {
                for &(v, _) in &comp.ordered_children[u as usize] {
                    edges.push((u, v, ()));
                    stack.push(v);
                }
            }
            ...
            let mut mapping = |u: u32| -> u32 {
                *mapping.entry(u).or_insert_with(|| {
                    let res = idx;
                    idx += 1;
                    res
                })
            ...
            for u in edges.iter_mut().flat_map(|(u, v, _)| [u, v]) {
                *u = mapping(*u);
            }
            let m = idx as usize;

            let comp = tree_iso::AHULabelCompression::from_edges(m, edges, None);
            subtrees.push(comp);
        }

        (subtrees.len() >= 2 && subtrees.windows(2).all(|t| t[0].is_iso_to(&t[1])))
            .then(|| subtrees.len())
    };
