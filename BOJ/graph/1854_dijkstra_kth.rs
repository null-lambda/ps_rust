...
fn main() {
    ...
    // k-th shortest path
    let mut queue: BinaryHeap<_> = [(Reverse(0i32), 0)].into();
    let mut dist = vec![BinaryHeap::new(); n];
    dist[0].push(0i32);
    while let Some((Reverse(d), u)) = queue.pop() {
        for &(v, w) in neighbors.get(u as usize) {
            let d_new = d + w;
            if dist[v as usize].len() < k {
                dist[v as usize].push(d_new);
                queue.push((Reverse(d_new), v));
            } else {
                let mut d_old = dist[v as usize].peek_mut().unwrap();
                if d_new < *d_old {
                    *d_old = d_new;
                    queue.push((Reverse(d_new), v));
                }
    ...
    for u in 0..n {
        if dist[u].len() < k {
            ...
            writeln!(output, "{}", dist[u].peek().unwrap()).unwrap();
        }
    }
}
