...
fn main() {
    ...
    let sa = suffix_trie::suffix_array(s, |&b| b - b'a');
    let lcp = suffix_trie::lcp_array(s, |&b| b - b'a', &sa);

    let count = sa
        .iter()
        .zip(&lcp)
        .map(|(&sa, &lcp)| s.len() as u32 - sa as u32 - lcp as u32);
    let prefix: Vec<_> = count
        .scan(0, |acc, x| {
            *acc += x;
            Some(*acc)
        })
    ...
    for _ in 0..input.value() {
        ...
        let i = partition_point(0, s.len() as u32, |i| prefix[i as usize] <= k) as usize;
        if i == s.len() {
            writeln!(output, "-1").unwrap();
            continue;
        }

        let len = lcp[i as usize] as u64 + k as u64
            - if i > 0 {
                prefix[i as usize - 1] as u64
            } else {
                0
            };
        let substr = &s[sa[i as usize] as usize..][..=len as usize];
        writeln!(output, "{}", unsafe {
            std::str::from_utf8_unchecked(substr)
        })
        .unwrap();
    }
}
