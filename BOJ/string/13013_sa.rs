...
fn main() {
    ...
    let sa: Vec<u32> = (0..n).map(|_| input.value()).collect();
    let mut sample = vec![0; n];
    for (rank, &len) in sa.iter().enumerate() {
        sample[len as usize] = rank as u32;
    }

    'outer: loop {
        for i in 0..n - 1 {
            if sample[sa[i] as usize] == sample[sa[i + 1] as usize] {
                continue;
            }
            let mut altered = sample.clone();
            altered[sa[i] as usize] = sample[sa[i + 1] as usize];

            let sa_recovered = suffix_trie::suffix_array(&altered, |&x| x as u8);
            if sa_recovered == sa {
                sample = altered;
                continue 'outer;
            }
    ...
    let n_unique = sample.iter().copied().collect::<HashSet<_>>().len();
    writeln!(output, "{}", n_unique).unwrap();
}
