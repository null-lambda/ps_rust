...
fn main() {
    ...
    for _ in 0..input.value() {
        ...
        for b in s.iter().copied().map(parse_char) {
            automaton.push(b);
        }
        for b in t.iter().copied().map(parse_char) {
            automaton.push(b);
        }

        let n_nodes = automaton.nodes.len();
        let mut degree = vec![1; n_nodes];
        let mut residual_length = vec![0; n_nodes];
        ...
        for mut u in 0..n_nodes as u32 {
            while degree[u as usize] == 1 {
                ...
                if node.is_rev_terminal() && node.rev_depth as usize > s.len() {
                    residual_length[u as usize] = s.len() as u32;
                }
                let len = node.rev_depth - automaton.nodes[p as usize].rev_depth;
                ans += residual_length[u as usize].min(len) as u64;
                residual_length[p as usize] = residual_length[p as usize]
                    .max(residual_length[u as usize].saturating_sub(len));
