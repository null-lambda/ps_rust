...
fn main() {
    ...
    let mut lcs_len = 0;
    let mut lcs_end = 0;
    for (i, b) in t.bytes().map(|b| b - b'a').enumerate() {
        while u != 0 && automaton.nodes[u as usize].children.get(b) == suffix_trie::UNSET {
            u = automaton.nodes[u as usize].rev_parent;
            len = automaton.nodes[u as usize].rev_depth as usize;
        }
        let c = automaton.nodes[u as usize].children.get(b);
        if c != suffix_trie::UNSET {
            u = c;
            len += 1;
        }
        if len > lcs_len {
    ...
    let lcs = &t.as_bytes()[lcs_end - lcs_len + 1..=lcs_end];
    writeln!(output, "{}", lcs_len).unwrap();
    writeln!(output, "{}", unsafe { std::str::from_utf8_unchecked(lcs) }).unwrap();
}
