...
const NEG_INF_64: i64 = i64::MIN / 3;
...
fn main() {
    ...
    let hld = hld::HLD::from_edges(n, mst_edges.iter().map(|&(u, v, _)| (u, v)), 0, false);
    ...
    let weights_even = SegTree::from_iter(
        n,
        weights
            .iter()
            .map(|&w| if w % 2 == 0 { w } else { NEG_INF }),
        MaxOp,
    );
    let weights_odd = SegTree::from_iter(
        n,
        weights
            .iter()
            .map(|&w| if w % 2 != 0 { w } else { NEG_INF }),
    ...
    let mut delta = -NEG_INF_64;
    for (u, v, w) in residual_edges {
        let mst_weights = if w % 2 == 0 {
            &weights_odd
        } else {
            &weights_even
        };
        let mut to_remove = NEG_INF;
        hld.for_each_path(u as usize, v as usize, |u, v, is_u_lca| {
            let mut su = hld.segmented_idx[u] as usize;
            let sv = hld.segmented_idx[v] as usize;
            if is_u_lca {
                su += 1;
            }
            to_remove = to_remove.max(mst_weights.query_range(su..sv + 1));
        });
        delta = delta.min(w as i64 - to_remove as i64);
    }
    let mut sub_mst_len = mst_len + delta as i64;
    if sub_mst_len >= -NEG_INF_64 {
        sub_mst_len = -1;
    }

    if mst_len % 2 == 0 {
        std::mem::swap(&mut mst_len, &mut sub_mst_len);
    }
    writeln!(output, "{} {}", mst_len, sub_mst_len).unwrap();
}
