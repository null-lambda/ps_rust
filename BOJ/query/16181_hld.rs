...
fn main() {
    ...
    let mut color_freq = vec![0i32; c + 1];
    let mut count_of_freq = vec![0i32; n];
    count_of_freq[0] = c as i32;

    let mut chain_agg = vec![ChainAgg::default(); n];
    for _ in 0..q {
        let u = input.value::<usize>() - 1;
        let c_new = input.value::<u32>();
        let m = input.value::<usize>();

        let mut inc = |color: u32, delta: i32| {
            count_of_freq[color_freq[color as usize] as usize] -= 1;
            color_freq[color as usize] += delta;
            count_of_freq[color_freq[color as usize] as usize] += 1;
        };

        let mut group = vec![];
        hld.for_each_path(u as usize, root as usize, |top, end, is_top_lca| {
            let prefix_len = (sid(end) - sid(top)) as u32 + 1 - is_top_lca as u32;
            if prefix_len == 0 {
                return;
            }

            chain_agg[top as usize]
                .overwrite_prefix(c_new, prefix_len - 1, |c, len| group.push((c, len)));
            ...
            if !group.is_empty() {
                ...
                for (color, len) in group.drain(..) {
                    inc(color, -(len as i32));
                }
            }

            inc(c_new, prefix_len as i32);
        });

        writeln!(output, "{}", count_of_freq[m as usize]).unwrap();
    }
}
