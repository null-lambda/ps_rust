...
struct AdditiveNode {
    sum: i64,
    link: wbtree::persistent::Link<AdditiveNode>,
}
...
impl wbtree::persistent::MonoidalReducer<AdditiveNode> for AdditiveNode {
    type X = i64;
    fn proj(u: &AdditiveNode) -> i64 {
        u.sum
    }
    fn id() -> i64 {
        0
...
impl wbtree::persistent::IntrusiveNode for AdditiveNode {
...
impl wbtree::persistent::NodeSpec for AdditiveNode {
    fn push_down(&mut self) {}
    fn pull_up(&mut self) {
        if let Some([left, right]) = self.link.children.as_ref() {
...
fn pow(
    mut u: wbtree::persistent::Rc<AdditiveNode>,
    k: usize,
) -> wbtree::persistent::Rc<AdditiveNode> {
    for _ in 0..k {
        u = wbtree::persistent::WBForest::merge_nonnull(u.clone(), u);
    }
    u
}
...
fn main() {
    ...
    type Forest = wbtree::persistent::WBForest<AdditiveNode>;
    let xs_orig = Forest::collect_from((0..n).map(|_| AdditiveNode::leaf(input.value()))).unwrap();
    ...
    for _ in 0..m {
        ...
        match cmd {
            "1" => {
                let ans = Forest::query_range::<AdditiveNode>(&mut xs, l..r + 1);
                writeln!(output, "{}", ans).unwrap();
            }
            "2" => {
                let k: usize = input.value();

                let (rest, right) = Forest::split_nonnull(xs, r + 1);
                let (left, _) = Forest::split_nonnull(rest.unwrap(), l);
                let (_, sample) = Forest::split(left.clone(), l - k);

                let n_samples = (r + 1 - l).div_ceil(k);
                let log2 = (usize::BITS - 1 - usize::leading_zeros(n_samples.next_power_of_two()))
                    as usize;
                let (extended_sample, _) =
                    Forest::split_nonnull(pow(sample.unwrap(), log2), r + 1 - l);
                xs = Forest::merge(Forest::merge(left, extended_sample), right).unwrap();
            }
            "3" => {
                let (rest, right) = Forest::split_nonnull(xs, r + 1);
                let (left, _) = Forest::split_nonnull(rest.unwrap(), l);

                let (mid_orig, _) = Forest::split_nonnull(xs_orig.clone(), r + 1);
                let (_, mid_orig) = Forest::split_nonnull(mid_orig.unwrap(), l);

                xs = Forest::merge(Forest::merge(left, mid_orig), right).unwrap();
            }
            _ => panic!(),
        }
    }
