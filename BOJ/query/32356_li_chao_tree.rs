...
pub fn main() {
    ...
    for _ in 0..n {
        ...
        let x_min = -x;
        let intercept = c * y - x * x;
        let slope = -2 * x;
        lines.push((x_min, intercept, slope))
    }
    lines.sort_unstable();

    // Do a offline sweeping
    let mut queries: Vec<_> = (0..q as u32).map(|i| (input.value::<i64>(), i)).collect();
    queries.sort_unstable();

    const X_MAX: i64 = 1_000_000_000;
    let mut cht = cht::LiChaoTree::new(-X_MAX..=X_MAX);
    let mut ans = vec![0.0; q];
    let mut lines = lines.into_iter().peekable();
    for (p, i) in queries {
        // Add all lines satisfying x_min <= p
        while let Some((_, intercept, slope)) = lines.next_if(|&(x_min, _, _)| x_min <= p) {
            cht.insert(cht::Line::new(slope, intercept));
        }

        let y = cht.eval(p) - p * p;
        if y > 0 {
            ans[i as usize] = y as f64 / c as f64;
        }
    }
