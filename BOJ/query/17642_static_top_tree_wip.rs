...
pub mod static_top_tree {
    pub mod rooted {
        ...
        pub trait ClusterCx: Sized {
            ...
            // Make a leaf compress cluster without rake edge.
            ...
            // Lazy propagation (Do it yourself)
            const LAZY: bool;
            fn push_down(
                &self,
...
fn main() {
    ...
    debug::with(|| {
        stt.debug_chains(|c, is_chain_top| {
            if is_chain_top {
                print!("chain top ");
            } else {
                print!("  ");
    ...
    for _ in 0..q {
        ...
        stt.modify(b as usize, |w| *w = w_new);
        ans = stt.sum_all().diameter;
