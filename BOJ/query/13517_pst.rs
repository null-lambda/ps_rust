...
pub mod segtree {
    ...
    pub mod persistent {
        ...
        impl<M: Monoid> NodePool<M> {
            ...
            pub fn get(&mut self, mut node: NodeRef, idx: usize) -> M::X {
                let (mut start, mut end) = (0, self.n);
                while end - start > 1 {
                    let mid = (start + end) / 2;
                    if idx < mid {
            ...
            M::X: Ord,
        {
            pub fn nth(
                &self,
                mut node_left: NodeRef,
                mut node_right: NodeRef,
                mut node_join: NodeRef,
                mut node_join_parent: NodeRef,
                mut bound: M::X,
            ) -> usize {
                let (mut start, mut end) = (0, self.n);
                while end - start > 1 {
                    ...
                    if bound < acc_left {
                        ...
                        branch = 1;
                        start = mid;
                        bound = self.monoid.sub(&bound, &acc_left);
                    }

                    node_left = self.nodes[node_left as usize].children[branch];
                    node_right = self.nodes[node_right as usize].children[branch];
                    node_join = self.nodes[node_join as usize].children[branch];
                    node_join_parent = self.nodes[node_join_parent as usize].children[branch];
                }
                start
            }
        }
...
fn dfs_pst(
    ...
    let old = pool.query_range(prev_root, xs[u - 1] as usize..xs[u - 1] as usize + 1);
    freq[u] = pool.set(prev_root, xs[u - 1] as usize, old + 1);
    for &v in &neighbors[u] {
        if v == parent {
            continue;
        }
