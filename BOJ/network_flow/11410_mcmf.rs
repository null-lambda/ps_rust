...
fn main() {
    ...
    network.set_supply(0, p as i32);
    network.set_demand(n - 1, p as i32);
    const INF_CAP: i32 = 1 << 28;
    for i in 0..n - 1 {
        network.add_edge([i, i + 1], 0, INF_CAP, 0);
    }
    ...
    for i in 0..n {
        for j in i + 1..n {
            let score: i32 = input.value();
            network.add_edge([i, j], 0, cap[i][j], -score);
        }
    }

    network.min_cost_max_flow();
    let ans = -network.circulation_cost();
