...
fn main() {
    ...
    for i in 0..n {
        network.add_edge(src, i, 0, 2);
        network.add_edge(i + n, sink, 0, 1);

        for j in 0..n {
            if if ps[i] == ps[j] {
                i < j
            } else {
                i != j && ps[i][0] >= ps[j][0] && ps[i][1] >= ps[j][1] && ps[i][2] >= ps[j][2]
            } {
                network.add_edge(i, j + n, 0, 1);
            }
        }
    }

    let max_flow = network.max_flow(src, sink);
    let ans = n as i32 - max_flow;
