...
fn main() {
    ...
    let sink = input.value::<usize>() - 1;

    const INF_FLOW: i64 = 1 << 42;
    ...
    let supersrc = n * 2;
    let supersink = n * 2 + 1;
    let mut net = network_flow::NetworkSimplex::with_size(n * 2 + 2);
    net.set_supply(supersrc, INF_FLOW / 2);
    net.set_demand(supersink, INF_FLOW / 2);
    let mut net_edges = vec![];
    let mut add_edge = |u, v, cap| {
        net_edges.push((u, v, net.add_edge([u, v], 0, cap, 1)));
    };

    add_edge(supersrc, v_in(src), INF_FLOW);
    add_edge(v_out(sink), supersink, INF_FLOW);
    for u in 0..n {
        let cap: i64 = input.value();
        add_edge(v_in(u), v_out(u), cap);
    }

    for _ in 0..m {
        let u = input.value::<usize>() - 1;
        let v = input.value::<usize>() - 1;
        add_edge(v_out(u), v_in(v), INF_FLOW);
        add_edge(v_out(v), v_in(u), INF_FLOW);
    }

    let min_cut = net.min_cost_max_flow();
    let mut net_neighbors = vec![vec![]; n * 2 + 2];
    let mut visited = vec![false; n * 2 + 2];
    for (u, v, e) in net_edges {
        if net.edges[e].flow < net.edges[e].upper {
            net_neighbors[u].push(v);
        }
        if net.edges[e].flow > 0 {
            net_neighbors[v].push(u);
        }
    }

    let mut stack = vec![supersrc];
    visited[supersink] = true;
    while let Some(u) = stack.pop() {
        for &v in &net_neighbors[u] {
            if visited[v] {
                continue;
            }
            visited[v] = true;
    ...
    for u in 0..n {
        if visited[v_in(u)] && !visited[v_out(u)] {
