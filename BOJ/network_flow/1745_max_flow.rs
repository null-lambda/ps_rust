...
pub fn main() {
    ...
    let mut srcs = vec![0; n_verts];
    let mut sinks = vec![0; n_verts];

    for u in 0..n_verts {
        srcs[u] = input.value::<i64>();
        sinks[u] = input.value::<i64>();
    }
    let total_src: i64 = srcs.iter().sum();

    const INF_DIST: u64 = u64::MAX / 3;
    let mut dist_mat = vec![INF_DIST; n_verts * n_verts];
    for _ in 0..n_edges {
        let u = input.value::<usize>() - 1;
        let v = input.value::<usize>() - 1;
        let d = input.value::<u64>();
        dist_mat[u * n_verts + v] = dist_mat[u * n_verts + v].min(d);
        dist_mat[v * n_verts + u] = dist_mat[v * n_verts + u].min(d);
    }
    ...
    for k in 0..n_verts {
        for i in 0..n_verts {
            for j in 0..n_verts {
                dist_mat[i * n_verts + j] = dist_mat[i * n_verts + j]
                    .min(dist_mat[i * n_verts + k] + dist_mat[k * n_verts + j]);
            }
        }
    }

    let satistiable = |time_bound: u64| {
        let mut network = network_flow::NetworkSimplex::with_size(n_verts * 2);

        let enter = |u| u;
        let exit = |u| u + n_verts;
        for u in 0..n_verts {
            network.set_supply(enter(u), srcs[u]);
            network.set_demand(exit(u), sinks[u]);
        }

        const INF_FLOW: i64 = 1 << 48;
        for u in 0..n_verts {
            network.add_edge([enter(u), exit(u)], 0, INF_FLOW, 0);
        }
        for u in 0..n_verts {
            for v in 0..n_verts {
                if dist_mat[u * n_verts + v] <= time_bound {
                    network.add_edge([enter(u), exit(v)], 0, INF_FLOW, 0);
                }
            }
        }

        network.min_cost_max_flow() == total_src
    };

    const MAX_TIME: u64 = 1_000_000_000u64 * 1500;
    let ans = partition_point(0, MAX_TIME, |t| !satistiable(t));
    if ans == MAX_TIME {
