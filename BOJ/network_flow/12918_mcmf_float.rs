...
pub mod network_flow {
    ...
    impl NetworkSimplex {
        ...
        pub fn circulation_cost(&self) -> Cost {
            (0..self.n_edges)
                .map(|e| self.edges[e].flow as Cost * self.edges[e].cost)
                .sum()
        }
        ...
        fn validate_flow(&self) {
            for e in 0..self.n_edges {
                ...
                debug_assert!(
                    self.edges[e].flow == self.edges[e].lower || self.reduced_cost(e) <= 0.0
                );
                debug_assert!(
                    self.edges[e].flow == self.edges[e].upper || self.reduced_cost(e) >= 0.0
                );
            }
        }
        ...
        fn signed_reduced_cost(&self, e: usize) -> Cost {
            self.edges[e].state as i8 as Cost * self.reduced_cost(e)
        }

        fn run(&mut self) {
            let mut artif_cost = EPS;
            for e in 0..self.n_edges {
                let [u, v] = self.edges[e].nodes;
                self.edges[e].flow = 0;
        ...
        fn select_pivot_edge(&mut self) -> Option<usize> {
            ...
            let mut signed_reduced_cost = 0.0;
            ...
            for count in 0..self.n_verts + self.n_edges {
                ...
                if count % self.block_size == self.block_size - 1 && signed_reduced_cost < 0.0 {
                    break;
                }
            }
            let res = in_arc;
        ...
        fn pivot(&mut self, in_arc: usize) {
            ...
            debug_assert_ne!(potential_delta, 0.0);
...
fn main() {
    ...
    let mut left = vec![];
    let mut right = vec![];
    let mut net = network_flow::NetworkSimplex::with_size(n * 2);
    for _ in 0..n {
    ...
    for i in 0..n {
        net.set_supply(i, 1);
        net.set_demand(i + n, 1);
    }
    for i in 0..n {
        for j in 0..n {
            let cost = (left[i][0] - right[j][0]).hypot(left[i][1] - right[j][1]);
            net.add_edge([i, j + n], 0, 1, cost);
        }
    }
    let mf = net.min_cost_max_flow();
    let mut ans = 0.5 * net.circulation_cost();
    ans = (ans * 1000.0).round() / 1000.0;
    writeln!(output, "{:.3}", ans).unwrap();
}
