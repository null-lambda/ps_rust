...
fn main() {
    ...
    let mut grid = vec![vec![b'.'; 2 * n + 1]; 2 * n + 1];
    for i in 1..=n * 2 - 1 {
        let row = input.token().as_bytes();
        grid[i][1..=n * 2 - 1].copy_from_slice(row);
    }

    let mut network = network_flow::NetworkSimplex::with_size(n_verts);
    for i in 0..n + 1 {
        for j in 0..n + 1 {
            let u = i * (n + 1) + j;
            if (i + j) % 2 == 0 {
                network.set_supply(u, 3);
            } else {
                network.set_demand(u, 3);
            }
        }
    }
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            if j > 0 {
                let v = u + (n + 1);
                let flow_lb = if grid[gi + 1][gj] == b'.' { 0 } else { 1 };
                if parity == 0 {
                    network.add_edge([u, v], flow_lb, 1, 1);
                } else {
                    network.add_edge([v, u], flow_lb, 1, 1);
                }
            }
            if i > 0 {
                let v = u + 1;
                let flow_lb = if grid[gi][gj + 1] == b'.' { 0 } else { 1 };
                if parity == 0 {
                    network.add_edge([u, v], flow_lb, 1, 1);
                } else {
                    network.add_edge([v, u], flow_lb, 1, 1);
    ...
    writeln!(output, "{}", max_flow + 1).unwrap();
}
