...
fn main() {
    ...
    let r = (n.max(m) + 1).next_power_of_two() * 4;

    let p: u64 = 9223372036737335297;
    let mod_op = num_mod::MontgomeryU64::new(p);
    let gen = mod_op.transform(3);
    assert_eq!(0, (p - 1) % r as u64);
    let proot = mod_op.pow(gen, (p - 1) as u64 / r as u64);
    let proot_inv = mod_op.inv(proot);

    assert_eq!(mod_op.pow(gen, p - 1), mod_op.one());
    assert_eq!(mod_op.pow(proot, r as u64), mod_op.one());

    let mut xs: Vec<u64> = (0..n + 1)
        .map(|_| mod_op.transform(input.value()))
        .collect();
    let mut ys: Vec<u64> = (0..m + 1)
        .map(|_| mod_op.transform(input.value()))
        .collect();
    xs.resize(r, mod_op.zero());
    ys.resize(r, mod_op.zero());
    ...
    ntt::radix4(&mod_op, proot_inv, &mut xs);

    let r_inv = mod_op.inv(mod_op.transform(r as u64));
    let mut xor = 0;
    for &x in &xs {
        let x = mod_op.reduce(mod_op.mul(x, r_inv));
        xor ^= x;
    }
    writeln!(output, "{}", xor).unwrap();
}
