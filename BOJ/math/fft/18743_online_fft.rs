...
fn main() {
    ...
    // T(x) := x + (T*T)(x), T(0) = 0
    // (T*T)_l := sum_{i+j=l,i<=j+k} T_i T_j
    //          = (1/2) (T^2)_l + (1/2) [l is even] T_{l/2}^2 + sum_{i+j=l,j<i<=j+k} T_i T_j
    //
    // T(x) = x S(x)
    // => S(x) = 1 + x (S * S)(x)
    //
    ...
    let mut s = vec![M::one()];
    let mut s2 = online_conv::Mul::default();

    let inv2 = M::from(2u32).inv();
    for i in 1..n + 1 {
        let mut acc = M::zero();
        acc += inv2 * s2.push(s[i - 1], s[i - 1]);
        if (i - 1) % 2 == 0 {
            acc += inv2 * s[(i - 1) / 2] * s[(i - 1) / 2];
        }

        for p in (i + 1) / 2..=(i - 1).min((i - 1 + k) / 2) {
            let q = i - 1 - p;
            acc += s[p] * s[q];
        }
