...
fn main() {
    ...
    const EARLY_ADAPTOR: usize = 1;

    let mut dp = vec![[0u32, 1]; n];
    for &u in order[1..].iter().rev() {
        let (p, ()) = parent[u as usize];
        dp[p as usize][NORMAL] += dp[u as usize][EARLY_ADAPTOR];
        dp[p as usize][EARLY_ADAPTOR] += dp[u as usize][NORMAL].min(dp[u as usize][EARLY_ADAPTOR]);
    }
    let ans = dp[root][NORMAL].min(dp[root][EARLY_ADAPTOR]);
