...
fn main() {
    ...
    let m: usize = input.u32() as usize;

    let root = 0;
    let edges = (0..n as u32).map(|i| (i, input.u32() - 1, ())).skip(1);
    ...
    for &u in &order[1..] {
        let (p, _) = parent[u as usize];
        dp[u as usize] += dp[p as usize];
    }

    for u in 0..n {
        write!(output, "{} ", dp[u]).unwrap();
    }
}
