...
fn main() {
    ...
    let mut fs = [Poly::<M>::one(), Poly::one()];
    for i in 1..=n_bound {
        let [mut e_shift, mut o_shift] = fs.clone();
        e_shift.mul_xk_in_place(i);
        e_shift.mod_xk_in_place(n_bound + 1);
        o_shift.mul_xk_in_place(i);
        o_shift.mod_xk_in_place(n_bound + 1);

        let [e, o] = &mut fs;
        *e += &e_shift;
        *o -= &o_shift;
    }
    let fs_inv = [fs[0].inv_mod_xk(n_bound + 1), fs[1].inv_mod_xk(n_bound + 1)];
    ...
    for _ in 0..input.value() {
        ...
        let ans = if k > 0 {
            let c = |j: usize| {
                ((n + 1).saturating_sub(fs[j].len()).div_ceil(k)..=(n / k).min(fs_inv[j].len() - 1))
                    .map(|l| fs[j].0[n - k * l] * fs_inv[j].0[l])
                    .fold(M::zero(), |acc, x| acc + x)
            };
            c(0) + c(1)
        } else {
            let c = |j: usize| fs[j].0.get(n).copied().unwrap_or_default();
            c(0) + c(1)
        } * M::from(2u8).inv();
