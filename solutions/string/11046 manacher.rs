...
// manachers algorithm
fn palindrome_radius<T: Eq>(text: Vec<T>) -> Vec<usize> {
    let n = text.len();
    let mut i = 0;
    let mut radius = 0;
    let mut radii = vec![];
...
fn main() {
    ...
    let m = input.value();
    for _ in 0..m {
        let s = idx(input.value());
        let t = idx(input.value());
        let (center, radius) = ((s + t) / 2, (t - s) / 2);
        let result: bool = radius <= radii[center];
        writeln!(output_buf, "{}", result as u32);
    }
