...
fn main() {
    ...
    let a: Vec<i64> = (0..n).map(|_| input.value()).collect();
    let winner = (0..n).max_by_key(|&i| a[i]).unwrap();

    let h: Vec<_> = (0..n).map(|_| input.value()).collect();
    let l: Vec<_> = (0..n).map(|_| input.value()).collect();

    let mut network = network_flow::NetworkSimplex::with_size(2 * n + 2);
    let src = 2 * n;
    let sink = 2 * n + 1;

    network.set_supply(src, n as i64 - 1);
    network.set_demand(sink, n as i64 - 1);

    for i in 0..n {
        if i == winner {
            network.add_edge([src, i], 0, l[i], h[i]);
        } else {
            network.add_edge([src, i], 0, l[i] - 1, h[i]);
        }

        network.add_edge([i + n, sink], 0, 1, h[i]);
    }

    for i in 0..n {
        for j in 0..n {
            if a[i] > a[j] {
                network.add_edge([i, j + n], 0, 1, -(a[i] ^ a[j]));
            }
        }
    }

    let max_flow = network.min_cost_max_flow();
    debug_assert!(max_flow == n as i64 - 1);
    let min_cost = network.circulation_cost();
