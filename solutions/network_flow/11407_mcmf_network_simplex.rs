...
fn main() {
    use network_flow::NetworkSimplex;
    ...
    let mut solver = NetworkSimplex::new(n_dest + n_src);
    for u in 0..n_dest {
        solver.set_demand(u, input.value());
    }
    for u in 0..n_src {
        solver.set_supply(u + n_dest, input.value());
    }

    let mut cap = vec![vec![0; n_dest]; n_src];
    for u in 0..n_src {
        for v in 0..n_dest {
            cap[u][v] = input.value();
        }
    ...
    let mut cost = vec![vec![0; n_dest]; n_src];
    for u in 0..n_src {
        for v in 0..n_dest {
            cost[u][v] = input.value();
        }
    }

    for u in 0..n_src {
        for v in 0..n_dest {
            solver.add_edge([u + n_dest, v], 0, cap[u][v], cost[u][v]);
        }
    }

    let max_flow = solver.min_cost_max_flow();
    let min_cost = solver.circulation_cost();
