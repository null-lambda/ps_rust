...
pub mod network_flow {
    ...
    impl NetworkSimplex {
        ...
        pub fn min_cost_max_flow(&mut self) -> Flow {
            self.run();
            let res = (self.n_edges..self.n_edges + self.n_verts)
                .map(|e| self.edges[e].upper - self.edges[e].flow)
                .sum();
            self.edges.truncate(self.n_edges);
            res
        }
...
fn main() {
    ...
    for u in 0..n_src {
        for v in 0..n_dest {
            ...
            solver.add_edge([u + n_dest, v], 0, 100, cost);
        }
    }

    solver.min_cost_circulation();
    let ans = solver.circulation_cost();
