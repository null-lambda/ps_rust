...
pub mod network_flow {
    const UNSET: usize = i64::MAX as usize;
    ...
    impl NetworkSimplex {
        ...
        pub fn get_edge(&self, handle: EdgeHandle) -> &Edge {
            &self.edges[handle.0]
        }
...
fn main() {
    ...
    for _ in 0..input.value() {
        ...
        let grid: Vec<u8> = (0..h)
            .flat_map(|_| &input.token().as_bytes()[..w])
            .copied()
            .collect();

        let mut network = network_flow::NetworkSimplex::with_size(w * h + 2);
        let src = w * h;
        let sink = w * h + 1;

        let inf = 100_000_000;
        network.set_supply(src, inf);
        network.set_supply(sink, -inf);

        let mut ans = 0;
        for i in 0..h {
            for j in 0..w {
                let u = i * w + j;
                let is_boundary = i == 0 || i == h - 1 || j == 0 || j == w - 1;
                if is_boundary {
                    network.add_edge([u, sink], 0, inf, 0);
                    if grid[u] != b'#' {
                        ans += f;
                    }
                } else {
                    if grid[u] == b'#' {
                        network.add_edge([u, sink], 0, d, 0);
                    } else {
                        network.add_edge([src, u], 0, f, 0);
                    }
                }

                if i > 0 {
                    network.add_edge([u, u - w], 0, b, 0);
                    network.add_edge([u - w, u], 0, b, 0);
                }
                if j > 0 {
                    network.add_edge([u, u - 1], 0, b, 0);
                    network.add_edge([u - 1, u], 0, b, 0);
                }
            }
        }
