...
fn main() {
    ...
    for i in 0..n {
        let row = input.token();
        for (j, c) in row.bytes().take(n).enumerate() {
            adj_mat[i][j] = c == b'1';
        }
    }

    let k: usize = input.value();
    let mut states = vec![vec![Vacant; n]; k];
    for i in 0..n {
        for _ in 0..input.value() {
            let g: usize = input.value();
            states[g][i] = Occupied;
        }
    }
    ...
    for i in 0..n {
        for _ in 0..input.value() {
            let g: usize = input.value();
            if states[g][i] != Occupied {
                states[g][i] = Indeterminate;
            }
        }
    }
    ...
    let mut ans = 0;
    for state in states {
        let mut network = network_flow::NetworkSimplex::with_size(n);
        let inf = n as i32;
        for i in 0..n {
            match state[i] {
                Occupied => network.set_supply(i, inf),
                Vacant => network.set_demand(i, inf),
                Indeterminate => {}
            }
        }
