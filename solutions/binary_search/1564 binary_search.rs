...
fn main() {
    ...
    let (k, n): (_, u32) = (input.value(), input.value());
    let ls: Vec<u32> = (0..k).map(|_| input.value()).collect();

    let result = partition_point(1, 1u32 << 31, |x| {
        let cables: u32 = ls.iter().map(|&l| l / x).sum();
        n <= cables
    }) - 1;
